...
namespace FurnitureServiceBusinessLogic.BusinessLogics
{
    static class SaveToWord
    {
        ...
        public static void CreateDoc(WordInfo info)
        {
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                foreach (var furniture in info.Furnitures)
                {
                    docBody.AppendChild(CreateParagraph(new WordParagraph
                    {
                        Texts = new List<(string, WordParagraphProperties)> { (furniture.FurnitureName + " : ", new WordParagraphProperties { Size = "24", Bold = true }), (furniture.Price.ToString() , new WordParagraphProperties { Size = "24", Bold = false }) },
                        ParagraphProperties = new WordParagraphProperties
                        {
                            Size = "24",
        ...
        public static void CreateWarehouseDoc(WordInfoWarehouse info)
        {
              ...
              .Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                foreach (var warehouse in info.Warehouses)
                {
                    ...
                    cellPerson.Append(new TableCellProperties(
                        new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
                    cellPerson.Append(new Paragraph(new Run(new Text(warehouse.FullNameOfTheHead))));
